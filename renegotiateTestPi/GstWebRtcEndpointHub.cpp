...
void GstWebRtcEndpointHub::acceptCert() {

}

gboolean myAcceptCert(SoupMessage *m, GTlsCertificate * c, GTlsCertificateFlags f, gpointer u) { 
	Log::Inst().log("accc(): accepting cert!!!"); 
	return TRUE;
};
...
void GstWebRtcEndpointHub::connectToServerAsync() {
	...
	*/
	...
	GError *error = NULL;
	GTlsDatabase * tlsdb = g_tls_file_database_new("/etc/ssl/certs/ca-certificates.crt", &error);
	if ( error ) {
		Log::Inst().log("GstWebRtcEndpointHub::connectToServerAsync(): Oops!! error loading the TLS certificate database!!! Code: <<%d>> Msg: <<%s>>. This should not happen!", error->code, error->message);
		g_error_free(error);
		return;
	} else {
		Log::Inst().log("GstWebRtcEndpointHub::connectToServerAsync(): OK! Got the certs!");
	}

	session = soup_session_new();
	//g_object_set(session, "tls-interaction", NULL, "ssl-use-system-ca-file", TRUE, "https-aliases", https_aliases, NULL);
	g_object_set(session, "tls-database", tlsdb, "https-aliases", https_aliases, NULL);
	...
	auto accc = [](SoupMessage *m, GTlsCertificate * c, GTlsCertificateFlags f, gpointer u) { Log::Inst().log("accc(): accepting cert!!!"); return TRUE; };
	g_signal_connect(message, "accept-certificate", G_CALLBACK(myAcceptCert), NULL);
	...
	auto owsc = [](GObject * conn, GAsyncResult * res, gpointer msg) { GstWebRtcEndpointHub::Instance()->onWssServerConnected((SoupSession *)conn, res, (SoupMessage *)msg); };
        soup_session_websocket_connect_async (session, message, NULL, NULL, 0, NULL, (GAsyncReadyCallback)owsc, message);
...
void GstWebRtcEndpointHub::onWssServerConnected(SoupSession * session, GAsyncResult * res, SoupMessage * msg) {
	...
	Log::Inst().log("GstWebRtcEndpointHub::onWssServerConnected(): Async Connection attempt finished. <<%08X>>, Attempting WSS exchange.", session);
        ...
        if (error) {
		Log::Inst().log("GstWebRtcEndpointHub::onWssServerConnected(): WSS exchange failed. Code: <<%d>>, Msg: <<%s>>. Disconnecting from server.", error->code, error->message);
                g_error_free (error);
...
void GstWebRtcEndpointHub::onNewTransceiver(GstElement * webrtc, GstWebRTCRTPTransceiver * trans, gpointer userData) {
	...
	Log::Inst().log("GstWebRtcEndpointHub::onNewTransceiver(): A new transceiver has been created.");
}
...
#define CAMERA_SERIAL_NUMBER "11120627"
	...
	pipelineBinElement = gst_parse_launch ("webrtcbin bundle-policy=2 name=webrtcElement " 
		"audiotestsrc is-live=true wave=red-noise volume=0.1 ! audioconvert ! audioresample ! queue ! opusenc ! rtpopuspay ! "
		"queue ! capsfilter caps=" RTP_CAPS_OPUS " name=\"capsFilterBeforeWebRtcBin\" ! webrtcElement. "
		//"v4l2src name=cameraElement device=/dev/video0 ! " // Updated to use v4l2src for USB webcam
		"videotestsrc pattern=18 ! "
		"video/x-raw,format=(string)YUY2,width=(int)1920,height=(int)1080,framerate=(fraction)30/1 ! "
		"videoconvert ! " // Added videoconvert for compatibility
		"vp9enc name=encoder ! rtpvp9pay mtu=1300 pt=98 name=vp9payloader ! "
		"" RTP_CAPS_VP9 " ! webrtcElement. ", &error);
	...
	int I;
		...
		gint mline = -1;
		g_object_get(trans, "mlineindex", &mline);
		if ( mline == 1 ) {
			GstCaps * caps;
        ...
        "application/x-rtp,media=video,encoding-name=ulpfec,clock-rate=90000"
			...
			g_object_set(trans, "codec-preferences", gst_caps_ref(caps), NULL);
			gst_caps_unref(caps);
			/*
			g_object_set(trans,
		...
		gint direction = -1;
		g_object_get(trans, "direction", &direction);
		//Log::Inst().log("GstWebRtcEndpointHub::constructWebRtcPipeline(): Transceiver: <<%d>>, Directon: <<%d>>, MLine: <<%d>>, Codecs: <<%s>>.", I, direction, mline, gst_caps_to_string(trans->codec_preferences));
		Log::Inst().log("GstWebRtcEndpointHub::constructWebRtcPipeline(): Transceiver: <<%d>>, Directon: <<%d>>, MLine: <<%d>>, Codecs: <<%s>>.", I, direction, mline, "huh");
        }
...
void GstWebRtcEndpointHub::turnOffAudioInWebRtcPipeline() {
	...
	GArray *transceivers = NULL;
		...
		gint direction = -1;
		g_object_get(trans, "direction", &direction);
		Log::Inst().log("turnOffAudioInWebRtcPipeline(): Got a transceiver. %d %d", mline, direction);
	}
...
void GstWebRtcEndpointHub::turnOnAudioInWebRtcPipeline() {
	...
	GArray *transceivers = NULL;
		...
		gint direction = -1;
		g_object_get(trans, "direction", &direction);
		Log::Inst().log("turnOnAudioInWebRtcPipeline(): Got a transceiver. %d %d", mline, direction);
	}
