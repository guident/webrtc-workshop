...
export class User2Service {
  ...
  constructor(private authService: CraigAuthenticateService) {
    // user2Var.myep.getLocalAudioStream();
  }
  ...
  SayHelloToService(){  
    ...
    // // AA: Do authentication like in Service 1:
    // this.user2Var.setCredentials(this.authService.getAuthUserEmail(), this.authService.getAuthAccessToken())
  ...
  async engageTheVehicle() {
    ...
    console.log("ANDY: ", this.user2Var.getVehicle31ConnectionId());
    if ( this.user2Var.getVehicle31ConnectionId() == "" ) return;
    await this.user2Var.getLocalMediaStream();
    // this.user2Var.myep.setOfferVideoPayloadTypeManipulations(98, 98, 98, 99, 100, 101); // error here: not a function
    this.user2Var.setOfferVideoPayloadTypeManipulations(98, 98, 98, 99, 100, 101);
